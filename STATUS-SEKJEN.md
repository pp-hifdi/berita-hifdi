...
- **SISTEM FOTO GILIR (restu Prinsipal/Sekjen):** bot harian BOLEH memakai koleksi 57 foto. Pemetaan kategori editorial → kategori foto (`PHOTO_BY_CATEGORY`) sudah ditambahkan di `config.py`. **Syarat 1 TERPENUHI:** audit & dokumentasi hak-pakai selesai (`docs/HAK-PAKAI-FOTO.md`) — **3 foto dikeluarkan**: `cat-07-001`, `cat-08-001` (by-nd) & `cat-15-001` (lisensi tak tercatat); 54 aktif (by 35, by-sa 16, pdm 3). Syarat 2 belum diterima. Integrasi `generate_article.py` belum dikerjakan (tunggu arahan).
